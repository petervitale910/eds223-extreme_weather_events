...
# Analysis
To start this analysis we need to mosaic the two files (overlapping boundaries), then check the crs
...
categories <- c("No Blackout", "blackout")
...
houses <- check_3083(houses)

blackout_final_roads <-  check_3083(blackout_final_roads)

check_crs(blackout_final_roads, houses)

blackout_buildings <- houses %>% 
  st_filter(y = st_union(blackout_final_roads))
...
# Joined cropped blackout with census tracts 
